import { Component, OnInit, EventEmitter, OnDestroy, ViewChild, ElementRef, ChangeDetectorRef, Inject } from '@angular/core';
...
@Component({
  selector: 'app-destination-list-modal',
  templateUrl: './destination-list-modal.component.html',
  styleUrls: ['./destination-list-modal.component.scss'],
})
export class DestinationListModalComponent implements OnInit,OnDestroy {
  ...
  constructor(private  originDestinationService: OriginDestinationService,
    ...
    private cd: ChangeDetectorRef,private httpClient: HttpClient,private globalService:GlobalService, private _bottomSheetRef: MatBottomSheetRef<DestinationListModalComponent>,
    private formBuilder: FormBuilder,) {
    this.isSearching = false;
    this.apiResponse = []
    //this.getpopcityindestination();
    // console.log('destination', data)
    // this.pcity = data['cityAndAirportDataList'];
  ...
  //       this.nullAirportList = false;
  //       this.nopopcity = false;
  //       this.globalService.searchGetAirport(text).subscribe(
  //         res => {
  //           this.nullAirportList = false;
  //           this.isSearching = false;
  //           this.nopopcity = false;
  ...
  //   // fromEvent(this.airportSearchInput.nativeElement, "keyup")
  //   //   .pipe(
  //   //     // get value
  //   //     map((event: any) => {
  ...
  //  console.log('on select destination',destination);
   this.selecteddestination = destination;
   this.originDestinationService.sendDestination(this.selecteddestination);
   let DestinationAirportCityName = destination.cityName
   localStorage.setItem("DestinationAirportCityName", DestinationAirportCityName);

   localStorage.setItem("Destination Airport Name", destination.airportName);
   localStorage.setItem("Destination Airport Code", destination.airportCode);
   localStorage.setItem("Destination countryID", destination.countryID);
  //  this.originDestinationService.sendDestination(destination);
 ...
 //   this.servicedata.unsubscribe();
  ...
  search(){
    this.destinations = this.destinations.filter(res=> {
      return res.airportCode.toLocaleLowerCase().match(this.airportCode.toLocaleLowerCase());
    })
  }
...
//////this code is for direct call of service to get
  // popularcity(){
  //   this.servicedata =  this.globalService.getpopularcity(22).subscribe(popcity=>{
  //       let citys = popcity['cityList']
  //       console.log(popcity);
  //       this.pcity = citys;
  //     })
  //   }
 ...
 ///this call is for get data before page load on click 
  // getpopcityindestination(){
  //   this.servicedata =  this.globalService.getapidest().subscribe(pcity=>{
  //    let citys = pcity['cityList'];
  //    this.pcity = citys;
  //   })
  // }
