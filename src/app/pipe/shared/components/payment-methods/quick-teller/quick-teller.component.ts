...
@Component({
  selector: 'app-quick-teller',
  templateUrl: './quick-teller.component.html',
  styleUrls: ['./quick-teller.component.scss'],
  providers: [MessageService],

})
export class QuickTellerComponent implements OnInit {
///////
...
//////
  ...
  isuccfFinal: any;
  quickTellerServiceCharge: string;
  quickTellersurchargeCap: string;
...
// if(!regName.test(value)){
 ...
 // console.log('this.fullname',this.fullname);
  ...
  requestBooking(){
   ...
   //// console.log("flight book req body of quickteller", reqbody);
    this.flightService.flightbook(reqbody).subscribe((res) => {
   //// console.log('res of quickteller',res);
  ...
  quickTellerSurcharge(){
    this.quickTellerServiceCharge =   sessionStorage.getItem("quickTellerServiceCharge");
  this.quickTellersurchargeCap =   sessionStorage.getItem("quickTellersurchargeCap");
 // console.log('quickTellerServiceCharge>>>>>>>>',this.quickTellerServiceCharge);
// console.log('quickTellersurchargeCap>>>>>>>>',this.quickTellersurchargeCap);

////cal 
this.percentCal = this.quickTellerServiceCharge;

this.isuccfTxnValue =  sessionStorage.getItem('isuccfTxnValue');
console.log('isuccfTxnValue',this.isuccfTxnValue);
if(this.percentCal != null){
...
if(this.surchargeAmount2 > +this.quickTellersurchargeCap){
 this.surchargeAmount2 =  +this.quickTellersurchargeCap;
...
// console.log( "price on foooter before ocf" , this.displayfareoneway );
 ...
 this.displayfareoneway = this.displayfareoneway   + ocf ;
 this.displayfareoneway  = Math.ceil(this.displayfareoneway);
...
let ocf:any =  this.surchargeAmount2;
localStorage.setItem('surchargeAmount',ocf);
console.log( "price on foooter before ocf" , this.cprice );

this.cprice = this.cprice + ocf ;
this.cprice = Math.ceil(this.cprice);
...
this.multiflightFare = this.multiflightFare  + ocf ;
//this.multiflightFare  = Math.ceil(this.multiflightFare );
...
///else of trip type
 ...
 // console.log('no surchrge is for qucikteller in lcl')
}
